...
export function toSharedPostProps(sp, me) {
  ...
  return {
    ...
    author: sp.author, media: sp.media.map((m) => ({ url: m.url, type: m.type, name: m.name, alt: m.alt })),
...
const storyReplyInclude = {
  select: {
    id: true, imageUrl: true, expiresAt: true,
    author: { select: { id: true, name: true, image: true } },
  },
};
...
export function toMessageProps(m, me) {
  return {
    ...
    storyReply: m.removed || !m.storyReply ? null : {
      id: m.storyReply.id, imageUrl: m.storyReply.imageUrl, author: m.storyReply.author,
      unavailable: m.storyReply.expiresAt.getTime() < Date.now(),
    },
    parent: m.parent ? {
