    ...
    <li className={isReply ? "ml-8" : ""}>
      <div className="group flex items-start gap-2">
        <span className="flex-1 text-sm">
          ...
          {comment.pending && <span className="ml-1 text-[11px] font-semibold text-amber-600">· pending review</span>}
          {pinned && <span className="ml-1 text-[11px] font-semibold text-brand">· 📌 pinned</span>}
          {comment.mediaUrl && (
          ...
          <span className="mt-0.5 flex items-center gap-3 text-[11px] text-gray-400">
            ...
            <button onClick={() => setReplying((v) => !v)} className="font-semibold hover:text-gray-600">Reply</button>
            {canPin && <button onClick={togglePin} className="font-semibold hover:text-brand">{pinned ? "Unpin" : "Pin"}</button>}
            <span className="opacity-0 transition group-hover:opacity-100">
              {!comment.mine && <button onClick={() => onReport(comment.id)} className="font-medium hover:text-brand">report</button>}
            </span>
        ...
        <button onClick={toggleLike} aria-label="Like comment" className="mt-0.5 shrink-0">
      ...
      {replying && (
        <form onSubmit={submitReply} className="ml-2 mt-1 flex items-center gap-2">
          ...
          />
          <button type="submit" disabled={!draft.trim() || busy} className="text-xs font-semibold text-brand disabled:text-gray-300">Post</button>
        </form>
      )}
