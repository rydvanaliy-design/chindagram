...
export default function ClubMemberRow({ clubId, member, iAmAdmin, isCreator, currentUserId }) {
  ...
  async function remove() {
    if (!window.confirm(`Remove ${user.name} from the club?`)) return;
    setBusy(true);
  ...
  return (
    <li className="flex items-center gap-3 px-4 py-3">
      ...
      <span className="min-w-0 flex-1">
        ...
        {role === "ADMIN" && <span className="text-xs font-semibold text-brand">Club admin</span>}
      </span>
      <RoleBadge role={user.role} />
      {iAmAdmin && user.id !== currentUserId && !isCreator && (
        <div className="flex shrink-0 gap-2 text-xs">
          {role === "ADMIN" ? (
            <button disabled={busy} onClick={() => setRole("MEMBER")} className="font-semibold text-gray-500 hover:text-brand disabled:opacity-50">Remove admin</button>
          ) : (
            <button disabled={busy} onClick={() => setRole("ADMIN")} className="font-semibold text-gray-500 hover:text-brand disabled:opacity-50">Make admin</button>
          )}
          <button disabled={busy} onClick={remove} className="font-semibold text-red-500 hover:underline disabled:opacity-50">Remove</button>
        </div>
      )}
    </li>
  );
